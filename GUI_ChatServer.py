...
class ChatserverMulti:
    ...
    def accept_client(self):
        while True:
            ...
            cth=Thread(target=self.receive_message,args=(c_socket,), daemon=True)    # 수신스레드, daemon=True쓰면 포트번호 고정(안바꿔도됨)
            cth.start()                                                 # 스레드 시작
    ...
    def receive_message(self,c_socket):
        while True:
            try:
                encoming_info = c_socket.recv(256)
                incoming_message = c_socket.recv(256)
                #if
                #raw_data = 리스트, 딕셔너리, int, str 등 모든 자료형
                #data = jason.dumps(raw_data)
                #c_socket.send(data.encoding())
                if not incoming_message: # 연결이 종료됨
                    break
            except:
                continue
            else: # 예외가 발생하지 않아 except절을 실행하지 않았을 경우 실행됨.
                encoming = eval(encoming_info.decode('utf-8')) # eval
